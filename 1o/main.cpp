...
void read2(vector<int> *v1, vector<int> *v2)
{
    ...
    sort(v1sorted.begin(), v1sorted.end()); // O(n log n)
    getline(cin, s2);
    istringstream input2(s2);
    while (input2 >> n)
    {
        if (binary_search(v1sorted.begin(), v1sorted.end(), n)) // O(log n)
        {
            (*v2).push_back(n); // O(1)
        }
    }
...
void findLargestSubsequences(vector<int> list, int *maxSize, int *nOfMaxSizeVectors)
{
    ...
    vector<int> max_size_list(size, 1), n_of_max_size_list(size, 1); // O(n)
